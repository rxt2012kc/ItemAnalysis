...
public class JsonParser {
	public static void main(String[] args) {
		...
		System.out.println("好评：" + reviewDis.get("好评"));
		System.out.println("中评：" + reviewDis.get("中评"));
		System.out.println("差评：" + reviewDis.get("差评"));
		System.out.println();
		...
		System.out.println("纸质书：" + formatDis.get("纸质书"));
		System.out.println("电子书：" + formatDis.get("电子书"));
		System.out.println();
		...
		// Iterator<HashMap.Entry<String, Integer>> iter =
		// map.entrySet().iterator();
		// while (iter.hasNext()){
		// HashMap.Entry<String, Integer> entry = (HashMap.Entry<String,
		// Integer>) iter.next();
	...
	public HashMap<String, Integer> reviewDistribution() {
		...
		for (int i = 0; i < Detail.size(); ++i) {
			if (Detail.get(i).getLabel().equals(GoodReview)) {
				reviewDis.put("好评", reviewDis.get("好评") + 1);
			} else if (Detail.get(i).getLabel().equals(MediumReview)) {
				reviewDis.put("中评", reviewDis.get("中评") + 1);
			} else if (Detail.get(i).getLabel().equals(PoorReview)) {
				reviewDis.put("差评", reviewDis.get("差评") + 1);
			}
		}
		return reviewDis;
	}
	...
	/*
	 * Count the tendency of good, medium, poor reviews changes with timeline
	 * reviewEmotionTrendTimeLine() get the num of good, medium, poor reviews
	 * changing with timeline.
	 */
	...
	public HashMap<String, Integer> formatDistribution() {
		...
		for (int i = 0; i < Detail.size(); ++i) {
			if (Detail.get(i).getFormat().equals(EBook)) {
				formatDis.put("纸质书", formatDis.get("纸质书") + 1);
			} else if (Detail.get(i).getFormat().equals(PaperBook)) {
				formatDis.put("电子书", formatDis.get("电子书") + 1);
			}
		}
		return formatDis;
	}
	...
	public class GoodDetail {
		...
		private GoodDetail(String date, String format, String review, String label) {
			this.Date = date;
			this.Format = format;
			this.Review = review;
			this.Label = label;
